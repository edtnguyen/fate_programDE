#!/usr/bin/env python
"""Sweep s_tau with multi-seed averaging for recovery."""
...
def main() -> None:
    ...
    ap.add_argument("--s-tau-values", default="1.0,0.7,0.5,0.3,0.1")
    ap.add_argument("--s-time", type=float, default=0.3)
    ap.add_argument("--s-guide", type=float, default=0.5)
    ...
    s_tau_values = _parse_list(args.s_tau_values, float)
        ...
        for seed in seeds:
            ...
            guide = fit_svi(
                *model_args,
                ...
                s_tau=float(s_tau),
                s_time=float(args.s_time),
                s_guide=float(args.s_guide),
                seed=seed,
            )
    ...
    for row in summary_rows:
        print(
            f"s_tau={row['s_tau']}: rmse_mean={row['rmse_mean']:.4f}, "
            f"r_mean={row['corr_mean']:.3f}, sign_mean={row['sign_mean']:.3f}, "
            f"rmse_avg={row['rmse_avg']:.4f}, r_avg={row['corr_avg']:.3f}, "
            f"sign_avg={row['sign_avg']:.3f}"
        )
