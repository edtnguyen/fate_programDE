...
def build_padded_guides(
    ...
    """
    required_cols = {"raw_guide_id", "guide_id", "gene_id"}
    missing = required_cols.difference(guide_map.columns)
    if missing:
        missing_str = ", ".join(sorted(missing))
        raise KeyError(f"guide_map missing required columns: {missing_str}")
    ...
    for _, row in guide_map.iterrows():
        ...
        if gid < 0 or gid > G:
            raise ValueError(f"guide_id {gid} is outside [0, {G}]")
        if gene < 0 or gene > L:
            raise ValueError(f"gene_id {gene} is outside [0, {L}]")
        if gid >= 0:
